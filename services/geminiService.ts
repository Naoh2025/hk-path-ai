...
export const getFreeAssessment = async (profile: UserProfile): Promise<AssessmentResult> => {
  ...
  2. 綜合評估術語選擇：【極具優勢】、【背景良好】、【具備潛力】、【需大幅優化】、【風險極高】。
  3. 針對「去年收入」點出高才 A 類在證明文件(稅單/審計報告)上的真實難點。
  4. 分析中必須包含對其「職業可持續性(續簽難易度)」的客觀預判。
  5. 風格保持冷靜、清醒，針對不合適的申請人直接「勸退」。
  6. 全文請使用【簡體中文】書寫，術語需符合內地閱讀習慣（例如：使用“學歷認證”、“社保”等詞彙）。
  7. 評估「進修轉身」可能性：
   - 即使目前職涯背景一般或收入不足，若客戶擁有學位、良好的語言基礎（如 IELTS 6.5以上）且有辦理預算（30萬 HKD 以上），請優先評估其通過「香港碩士留學（進修）」獲得身份的潛力。
   - 在分析中，應將「留學路徑」視為一種主動提升背景的策略，而非僅僅看目前的優才/高才分數。
   8. 邏輯優先級：
   - 若 TTPS（高才）和 QMAS（優才）條件不足，但具備預算和學歷，請在 recommendedPath 中大膽推薦「香港進修/留學」，並在分析中說明如何通過一年碩士拿到 IANG 簽證，實現身份反轉。
